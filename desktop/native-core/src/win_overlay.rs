...
// ---------------------------------------------------------------------------
// Onboarding step enum (mirrors runtime)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Overlay state (shared between render + runtime threads)
// ---------------------------------------------------------------------------
...
static OVERLAY_THREAD_STARTED: OnceLock<()> = OnceLock::new();
...
pub fn spawn_for_core(on_event: impl Fn(&'static str) + Send + 'static) {
    let _ = STATE.set(Arc::new(Mutex::new(OverlayState::default())));
    init_core_control_dispatcher(on_event);
    init_mouse_hook();
    spawn_window_thread();
}

fn init_core_control_dispatcher(on_event: impl Fn(&'static str) + Send + 'static) {
    ...
    std::thread::Builder::new()
        .name("yap-win-overlay-core-controls".into())
        .spawn(move || {
            while let Ok(command) = rx.recv() {
                match command {
                    ControlCommand::PillClick => on_event("pill_click"),
                    ControlCommand::TogglePause => on_event("pause"),
                    ControlCommand::Stop => on_event("stop"),
                }
            }
        })
        .expect("failed to spawn overlay core control thread");
}
...
fn init_mouse_hook() {
    if MOUSE_HOOK_STARTED.set(()).is_err() {
        return;
    }
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || unsafe {
            ...
            let hook = match hook {
                Ok(hook) => hook,
                Err(e) => {
                    crate::log::info(&format!("Win32 mouse hook failed: {e}"));
                    return;
                }
            };

            crate::log::info("Win32 mouse hook installed");
...
fn spawn_window_thread() {
    if OVERLAY_THREAD_STARTED.set(()).is_err() {
        post_overlay_update();
        return;
    }
